	...
	<Card.Root class="gap-0 overflow-hidden py-0">
		<PanelHead title="Web hygiene">
			{#if summary}
				<span class="tabular-nums"
					>{plural(evaluated, 'web host checked', 'web hosts checked')}</span
				>
		...
		{:else if evaluated === 0}
			<p class="p-5 text-sm text-muted-foreground">
				{plural(pending, 'web host', 'web hosts')} not yet evaluated.
			</p>
		{:else}
			<div class="grid grid-cols-1 md:grid-cols-2">
			...
			>
				{#if summary.clean > 0}
					...
					>
						{plural(summary.clean, 'host passes', 'hosts pass')} every check that applies
						<ChevronRight class="size-3.5" />
					</Button>
				{/if}
				{#if pending > 0}
					<span class="tabular-nums"
						>{plural(pending, 'web host', 'web hosts')} not yet evaluated</span
					>
				{/if}
				{#if passing.length}
					<span class="flex flex-wrap items-center gap-1.5">
						<span>Passing on every host it applies to</span>
							...
							<span class="flex w-full items-center gap-2">
								<span class="flex min-w-0 flex-1 flex-col">
									...
									<span class="truncate text-xs leading-4 text-muted-foreground">
										of {plural(r.count.applicable, 'host', 'hosts')} where it applies
									</span>
								</span>
