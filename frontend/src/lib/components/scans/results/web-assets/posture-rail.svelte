<script lang="ts">
	...
	let breakdown = $derived(hostBreakdown(summary));
	let cleanOn = $derived(selected.includes(POSTURE_NONE));
</script>
...
>
	<div class="flex h-10 items-center gap-2 border-b px-4">
		<span class="text-sm font-medium">Domain posture</span>
		{#if summary}
			<span class="text-xs text-muted-foreground tabular-nums">
				{breakdown.evaluated.toLocaleString()} under a checked zone
			</span>
		{/if}
		<Button
			variant="ghost"
			...
			aria-label="Close domain posture"
			onclick={onClose}
		>
			<X class="size-4" />
		</Button>
	...
	{:else if breakdown.evaluated === 0}
		<p class="p-4 text-xs text-muted-foreground">Not scanned</p>
	{:else}
		{@render section(PostureTone.WARNING, breakdown.warnings)}
		{@render section(PostureTone.INFO, breakdown.infos)}
		<div class="flex flex-col gap-1.5 border-t px-4 py-3 text-xs text-muted-foreground">
			...
			{#if breakdown.pending > 0}
				<span class="tabular-nums"
					>{plural(breakdown.pending, 'web asset', 'web assets')} under a zone not checked</span
				>
			{/if}
		</div>
	{/if}
...
{#snippet section(tone: PostureTone, rows: PostureRow[])}
