<script lang="ts">
	...
	const MODE_LABELS: Record<string, string> = { single: 'Single', rotating: 'Rotating' };
	...
	<div class="flex items-start justify-between">
		<div>
			...
			<p class="text-sm text-muted-foreground">
				Route scan traffic through proxies. The default applies to scan contexts that don't pick one.
			</p>
		</div>
		...
		<Card.Root class="border-dashed">
			<Card.Content class="flex flex-col items-center justify-center gap-3 py-16 text-center">
				...
				<div class="space-y-1">
					...
					<p class="text-xs text-muted-foreground">Add a proxy to route recon traffic through it.</p>
				</div>
	...
	{:else}
		<div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
			{#each proxies as proxy (proxy.id)}
				<Card.Root class="relative transition-all duration-200 {proxy.is_active ? 'ring-1 ring-border' : 'border-dashed opacity-75'}">
					<Card.Content class="p-5">
							...
							<div class="flex min-w-0 items-start gap-3">
								...
								<div class="min-w-0">
									<div class="flex items-center gap-2">
										...
										{#if proxy.is_default}
											<Badge variant="secondary" class="h-5 shrink-0 border-0 px-1.5 text-[10px]">Default</Badge>
										{/if}
									</div>
									<p class="mt-0.5 line-clamp-2 text-xs text-muted-foreground">
										{proxy.description || `${MODE_LABELS[proxy.mode] ?? proxy.mode} · ${proxy.endpoint_count} endpoint${proxy.endpoint_count === 1 ? '' : 's'}`}
									</p>
								</div>
							</div>
							<Switch
						...
						<div class="space-y-2">
							{#each proxy.endpoints.slice(0, 3) as ep (ep.url_masked)}
								<code class="block truncate font-mono text-xs text-muted-foreground">{ep.url_masked}</code>
							{/each}
						...
						<div class="mt-3 flex items-center gap-1.5">
							<Tooltip.Root>
								<Tooltip.Trigger>
									{#snippet child({ props })}
										<Button {...props} variant="ghost" size="sm" class="h-7 px-2 text-xs" onclick={() => openEdit(proxy)}>
		...
		<ScrollArea class="flex-1">
			...
			<div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
				<div class="space-y-1.5">
					<Label class="text-xs" for="proxy-name">Name</Label>
					<Input id="proxy-name" bind:value={formName} placeholder="Residential pool" class="h-9" disabled={saving} />
				</div>
				<div class="space-y-1.5">
					<Label class="text-xs">Mode</Label>
					<Select.Root type="single" bind:value={formMode}>
						<Select.Trigger class="h-9 w-full text-sm">{MODE_LABELS[formMode] ?? 'Single'}</Select.Trigger>
						<Select.Content>
							{#each PROXY_MODES as m (m)}
								<Select.Item value={m} label={MODE_LABELS[m]}>{MODE_LABELS[m]}</Select.Item>
							{/each}
						</Select.Content>
					</Select.Root>
				</div>
			...
			<div class="space-y-1.5">
				<Label class="text-xs" for="proxy-desc">Description <span class="text-muted-foreground">(optional)</span></Label>
				<Input id="proxy-desc" bind:value={formDescription} placeholder="What this proxy is for" class="h-9" disabled={saving} />
			</div>
			...
			<div class="space-y-3">
				...
				{#each formRows as row, i (i)}
					<div class="space-y-3 rounded-lg border bg-muted/30 p-3">
						<div class="grid grid-cols-1 gap-2.5 sm:grid-cols-[6.5rem_1fr_5.5rem_auto]">
							<div class="space-y-1">
								<Label class="text-[10px] uppercase tracking-wide text-muted-foreground">Scheme</Label>
								<Select.Root type="single" value={row.scheme} onValueChange={(v) => setRow(i, 'scheme', v ?? 'http')}>
									<Select.Trigger class="h-9 w-full text-sm">{SCHEME_LABELS[row.scheme] ?? 'HTTP'}</Select.Trigger>
									<Select.Content>
						...
						<div class="grid grid-cols-1 gap-2.5 sm:grid-cols-2">
							<div class="space-y-1">
								<Label class="text-[10px] uppercase tracking-wide text-muted-foreground">Username <span class="normal-case">(optional)</span></Label>
								<Input
									value={row.username}
							...
							<div class="space-y-1">
								<Label class="text-[10px] uppercase tracking-wide text-muted-foreground">Password <span class="normal-case">(optional)</span></Label>
								<Input
									type="password"
									value={row.password}
				...
				{/each}
				<Button variant="ghost" size="sm" class="h-8 gap-1.5 text-muted-foreground" onclick={addRow}>
			...
			<div class="flex items-center justify-between rounded-lg border px-4 py-3">
				...
				<Switch checked={formDefault} onCheckedChange={(v) => (formDefault = v)} disabled={saving} />
			</div>
		</div>
		</ScrollArea>
...
<Dialog.Root bind:open={deleteOpen}>
	<Dialog.Content class="sm:max-w-md">
		<Dialog.Header>
			<Dialog.Title>Remove proxy</Dialog.Title>
			<Dialog.Description>
				Are you sure you want to remove {deletingProxy?.name ?? 'this proxy'}? Scan contexts using it
				will fall back to direct traffic.
			</Dialog.Description>
		</Dialog.Header>
		<Dialog.Footer>
