...
function buildDomain(target: Target, detail: TargetDetailRead | null): TargetSummary {
	...
	const aRecs = recordsOfType(records, DnsRecordType.A);
	const aaaaRecs = recordsOfType(records, DnsRecordType.AAAA);
	const cnameRecs = recordsOfType(records, DnsRecordType.CNAME);
	const nsRecs = recordsOfType(records, DnsRecordType.NS);
	const mxRecs = recordsOfType(records, DnsRecordType.MX);
	const txtRecs = recordsOfType(records, DnsRecordType.TXT);
	...
	const nsProvider =
		detectProvider(nsHosts) ?? (whois?.nameservers ? detectProvider(whois.nameservers) : null);
	...
	for (const r of mxRecs)
		infra.push({
			kind: 'mx',
			value: r.value,
			note: r.priority != null ? `priority ${r.priority}` : undefined
		});
	...
	if (pendingFor(whoisStatus)) {
		...
		if (urg === 'expired')
			posture.push({
				key: 'whois',
				status: 'fail',
				label: 'Domain registration expired',
				detail: formatShortDate(expirationDate)
			});
		else if (urg === 'critical')
			posture.push({
				key: 'whois',
				status: 'warn',
				label: formatExpirationLabel(expirationDate),
				detail: 'renew soon to avoid loss'
			});
		else
			posture.push({
				key: 'whois',
				...
				detail: formatExpirationLabel(expirationDate)
			});
	}
		...
		posture.push(
			whois.dnssec
				...
				: {
						...
						detail: 'responses are not cryptographically signed'
					}
		);
	}
	...
	if (spf) {
		...
		posture.push({
			...
			status: 'warn',
			label: 'No SPF record',
			detail: 'mail is configured but sender policy is missing'
		});
	}
		...
		posture.push(
			locks.length
				? {
						key: 'lock',
						status: 'pass',
						label: `Registrar locks active (${locks.length})`,
						detail: 'transfer/delete protections set'
					}
	...
	if (pendingFor(dnsStatus))
		posture.push({ key: 'dns', status: 'pending', label: 'DNS resolution in progress' });
	...
	if (dns?.cdn)
		posture.push({
			...
			detail: 'origin shielded behind edge network'
		});
	...
	if (verifs.size)
		posture.push({
			key: 'verif',
			status: 'info',
			label: `${verifs.size} service verification${verifs.size > 1 ? 's' : ''}`,
			detail: [...verifs].join(', ')
		});
...
function buildNetwork(target: Target, detail: TargetDetailRead | null): TargetSummary {
	...
	if (t === TargetType.ASN) {
		...
		if (peerCount)
			vitals.push({
				...
				sub: `${upstream.length} up · ${downstream.length} down`,
				icon: 'route'
			});
		const topUpstream = upstream.length
			? [...upstream].sort((a, b) => (b.power ?? 0) - (a.power ?? 0))[0]
			: null;
		if (topUpstream)
			vitals.push({
				key: 'upstream',
				label: 'Primary upstream',
				value: `AS${topUpstream.neighbour_asn}`,
				mono: true,
				icon: 'radio',
				copy: `AS${topUpstream.neighbour_asn}`
			});
		if (rir)
			vitals.push({ key: 'rir', label: 'Registry (RIR)', value: rir.toUpperCase(), icon: 'flag' });
		...
		const holder =
			as?.holder || target.bgp?.holder || bgp?.prefix_overview?.[0]?.holder || whois?.name;
		if (originAsn)
			vitals.push({
				key: 'asn',
				label: 'Origin AS',
		...
		if (t === TargetType.IP_RANGE && bgp?.related_prefixes?.length)
			vitals.push({
				...
				value: String(bgp.related_prefixes.length),
				icon: 'hash'
			});
		if (rir)
			vitals.push({ key: 'rir', label: 'Registry (RIR)', value: rir.toUpperCase(), icon: 'flag' });
		if (whois?.assignment_type)
			vitals.push({
				key: 'assign',
				label: 'Assignment',
				value: whois.assignment_type,
				icon: 'fingerprint'
			});
		if (whois?.ip_version)
			vitals.push({
	...
	if (pendingFor(bgpStatus)) {
		...
		if (announced === false)
			posture.push({
				key: 'announced',
				status: 'warn',
				label: 'Not announced in the global routing table',
				detail: 'no visible BGP route'
			});
		else if (announced === true)
			posture.push({
				key: 'announced',
				status: 'pass',
				label: 'Announced in BGP',
				detail: 'visible in the global routing table'
			});
	}
	...
	if (abuse)
		posture.push({ key: 'abuse', status: 'pass', label: 'Abuse contact published', detail: abuse });
	else if (bgpStatus === TaskStatus.SUCCESS)
		posture.push({ key: 'abuse', status: 'info', label: 'No abuse contact found' });

	for (const ni of bgp?.network_info ?? [])
		infra.push({ kind: 'ipv4', value: ni.ip, note: ni.prefix });
