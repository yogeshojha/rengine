	...
	const state = $derived.by(() => {
		switch (status) {
			...
			case TaskStatus.QUERYING:
				return {
					dot: 'bg-chart-1/60 animate-pulse',
					time: 'Running',
					tip: 'Enrichment in progress'
				};
			case TaskStatus.PENDING:
				return { dot: 'bg-warning/50 animate-pulse', time: 'Queued', tip: 'Enrichment queued' };
			case TaskStatus.SKIPPED:
				return { dot: 'bg-muted-foreground/20', time: 'Skipped', tip: 'Not applicable' };
			default: {
				const t = queriedAt ? relativeTime(queriedAt) : '';
				return {
					dot: isStale ? 'bg-warning/60' : 'bg-muted-foreground/30',
					time: t,
	...
	<Tooltip.Trigger>
		{#snippet child({ props })}
			<Badge {...props} variant="outline" class="gap-1.5 border-border/60 font-normal">
				...
				{#if isStale}<TriangleAlert class="h-3 w-3 text-warning" />{/if}
			</Badge>
		{/snippet}
	</Tooltip.Trigger>
	<Tooltip.Content>
