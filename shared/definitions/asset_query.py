...
FIELDS: tuple[QueryField, ...] = (
    ...
    QueryField(
        ...
        type=FieldType.ENUM,
        group="Findings",
        description="The source of the flag: a rule, a correlation, or AI.",
        example="flagged:ai",
...
SERVICE_FIELDS: tuple[QueryField, ...] = (
    ...
    QueryField(
        ...
        type=FieldType.ENUM,
        group="Service",
        description="The source that observed the service.",
...
SERVICE_EXAMPLES: tuple[QueryExample, ...] = (
    ...
    QueryExample(
        ...
        description="Services requiring authentication",
        group="Web surface",
    ),
    QueryExample(
        query="is:passive",
...
ENDPOINT_FIELDS: tuple[QueryField, ...] = (
    ...
    QueryField(
        ...
        type=FieldType.STRING,
        group="Location",
        description="Directory the endpoint sits in. dir: matches the whole branch, dir= only that folder.",
        example='dir:"/admin/"',
    ...
    QueryField(
        ...
        type=FieldType.STRING,
        group="Location",
        description="Hostname the endpoint is on.",
    ...
    QueryField(
        ...
        type=FieldType.NUMBER,
        group="Location",
        description="Directory depth of the endpoint.",
        example="depth:>3",
    ),
    QueryField(
        name="class",
        type=FieldType.ENUM,
        group="Location",
        description="The kind of content the endpoint serves.",
        example="class:api",
        aliases=("kind",),
        values=ENDPOINT_CLASSES,
        facet="class",
    ),
    QueryField(
        name="param",
        type=FieldType.STRING,
        group="Parameters",
        description="Name of a query parameter the endpoint accepts.",
        example="param:redirect",
        aliases=("parameter",),
        facet="param",
    ),
    QueryField(
        ...
        type=FieldType.NUMBER,
        group="Parameters",
        description="Number of parameters the endpoint accepts.",
        example="params:>2",
    ),
    QueryField(
        name="interest",
    ...
    QueryField(
        ...
        type=FieldType.STRING,
        group="Response",
        description="HTTP method the endpoint was seen with.",
        example="method:POST",
    ),
    QueryField(
        name="source",
        type=FieldType.ENUM,
        group="Evidence",
        description="The source that discovered the endpoint. An endpoint may have several.",
        example="source:crawl",
    ...
    QueryField(
        ...
        type=FieldType.STRING,
        group="Evidence",
        description="The URL that linked to this one.",
        example="from:/sitemap.xml",
        aliases=("found_on", "referrer"),
    ),
    QueryField(
        name="seen",
        type=FieldType.DATE,
    ...
    QueryField(
        ...
        type=FieldType.ENUM,
        group="Findings",
        description="Severity of a vulnerability reported at this location.",
        example="vuln:critical",
        values=SEVERITY_ORDER,
    ),
    QueryField(
        ...
        type=FieldType.STRING,
        group="Findings",
        description="CVE reported at this location.",
        example="cve:CVE-2024",
    ),
    QueryField(
        name="is",
        type=FieldType.FLAG,
...
ENDPOINT_GROUP_DIMENSIONS: tuple[GroupDimension, ...] = (
    ...
    GroupDimension(
        key="dir",
        label="Directory",
        description="Endpoints in the same directory",
    ),
    GroupDimension(
        key="host",
        label="Host",
        description="Endpoints on the same host",
    ),
    GroupDimension(
        key="class",
        label="Kind",
    ...
    GroupDimension(
        key="ext",
        label="Extension",
        description="Endpoints with the same file extension",
    ),
    GroupDimension(
        key="status",
        label="Status",
        description="Endpoints answering with the same status",
    ),
    GroupDimension(
        key="source",
        label="Source",
        description="Endpoints found the same way",
    ),
    GroupDimension(
        key="param",
        label="Parameter",
        description="Endpoints accepting the same parameter",
    ),
    GroupDimension(
        key="interest",
        label="Interest",
    ...
    GroupDimension(
        key="tech",
        label="Technology",
        description="Endpoints running the same technology",
    ),
)

ENDPOINT_EXAMPLE_GROUPS: tuple[str, ...] = (
    "Attack surface",
    "Hidden surface",
    "Change",
    "API",
...
ENDPOINT_EXAMPLES: tuple[QueryExample, ...] = (
    QueryExample(
        query="is:param and is:live",
        description="Live endpoints that accept input",
    ...
    QueryExample(
        query="interest:traversal and is:live",
        description="Live endpoints taking a file or path parameter",
        group="Attack surface",
    ),
    QueryExample(
        query="is:sensitive",
        description="Credential, backup and version control paths",
        group="Attack surface",
        generic=True,
    ),
    QueryExample(
    ...
    QueryExample(
        query="interest:admin and status:200",
        description="Administrative paths answering without authentication",
        group="Attack surface",
    ),
    QueryExample(
        query="is:orphan",
    ...
    QueryExample(
        query="depth:>4 and is:live",
        description="Deeply nested live endpoints",
        group="Hidden surface",
    ),
    QueryExample(
        query="is:new",
    ...
    QueryExample(
        query="interest:api_doc",
        description="Swagger, OpenAPI and GraphQL descriptions of the API",
        group="API",
    ),
    QueryExample(
        query="class:api and is:param and not is:auth",
        description="API routes taking input with no auth wall",
        group="API",
    ),
    QueryExample(
        query="not is:probed",
    ...
    QueryExample(
        query="is:vulnerable",
        description="Endpoints a vulnerability scan reported a finding on",
        group="Evidence",
    ),
    QueryExample(
        query="interest:debug_endpoint",
        description="Actuator, metrics and diagnostic routes",
        group="Hygiene",
        generic=True,
    ),
    QueryExample(
    ...
    QueryExample(
        query="type:json and not class:api",
        description="JSON served outside the API surface",
        group="Hygiene",
    ),
)
...
SOFTWARE_FIELDS: tuple[QueryField, ...] = (
    ...
    QueryField(
        ...
        type=FieldType.ENUM,
        group="Software",
        description="Source of the reported version.",
    ...
    QueryField(
        ...
        type=FieldType.ENUM,
        group="Confidence",
        description="Verification level of the match.",
