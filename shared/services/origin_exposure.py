...
class OriginExposureService:
    ...
    def run(self, scan_id: UUID) -> OriginExposure:
        assets = self._assets(scan_id)
        if not assets:
    ...
    def _assets(self, scan_id: UUID) -> list[_Asset]:
        rows = (
            self.session.execute(select(*_COLUMNS).where(HttpAsset.scan_id == scan_id))
        ).all()
        out: list[_Asset] = []
        for r in rows:
            prints = {
    ...
    def _ports(self, scan_id: UUID) -> dict[str, list[int]]:
        rows = (
