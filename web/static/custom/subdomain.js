...
function mainCheckBoxSelected(checkbox) {
  if (checkbox.checked) {
    ...
    $('#subdomain_selected_count').text(interpolate('%(count)s Subdomains Selected x', {count: checkedCount()}, true));
  } else {
    uncheckSubdomains();
  }
}
...
function toggleMultipleSubdomainButton() {
  ...
  if (checked_count > 0) {
    $("[data-button=subdomain_btns]").removeClass("disabled");
    $('#subdomain_selected_count').text(interpolate('%(count)s Subdomains Selected x', {count: checked_count}, true));
  } else {
