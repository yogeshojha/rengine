let main_subdomain_suggestions = [
  'name',
  "page_title",
  "is_important",
  "http_url",
...
const filter_icon = `<span class="text-info"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-filter"><polygon points="22 3 2 3 10 12.46 10 19 14 21 14 12.46 22 3"></polygon></svg></span>`;
let linkTag = searchWrapper.querySelector("a");
let webLink;

inputBox.onclick = (event) => {
  emptyArray = main_subdomain_suggestions.filter((data)=>{
    return data.toLocaleLowerCase();
  });
  emptyArray = emptyArray.map((data)=>{
    // passing return data inside li tag
    return data = `<li class="text-dark"><div class="row"><div class="col-6">${filter_icon} ${data}</div><div class="col-6 text-dark">Filter subdomain that contains <span class="badge badge-dark">${data}</span></div></div></li>`;
  });
  searchWrapper.classList.add("active"); //show autocomplete box and put cross
